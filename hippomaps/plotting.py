...
def surfplot_canonical_foldunfold(
    ...
    **qwargs : dict, optional
    ...
    # hippunfold version
    version = "v1" if den in {"0p5mm", "1mm", "2mm", "unfoldiso"} else "v2"

    # load surfaces
    rh = read_surface(
        f"{resourcesdir}/canonical_surfs/{version}/tpl-avg_space-canonical_den-{den}_label-hipp_midthickness.surf.gii"
    )
    ru = read_surface(
        f"{resourcesdir}/canonical_surfs/{version}/tpl-avg_space-unfold_den-{den}_label-hipp_midthickness.surf.gii"
    )
    ru.Points = ru.Points[:, [1, 0, 2]]  # reorient unfolded

    if version == "v2" and unfoldAPrescale:
        raise Warning(
            "Outputs from hippunfold version 2 does not require rescaling, setting unfoldAPrescale to False"
        )
        unfoldAPrescale = False
    if unfoldAPrescale:
    ...
    if len(labels) == 2:
        ud = read_surface(
            f"{resourcesdir}/canonical_surfs/{version}/tpl-avg_space-unfold_den-{den}_label-dentate_midthickness.surf.gii"
        )
        hd = read_surface(
            f"{resourcesdir}/canonical_surfs/{version}/tpl-avg_space-canonical_den-{den}_label-dentate_midthickness.surf.gii"
        )
        ...
        # add to original
        npts = rh.n_points
        rh = mc.build_polydata(
            np.concatenate((rh.Points.copy(), hd.Points.copy())),
            cells=np.concatenate(
                (rh.GetCells2D().copy(), hd.GetCells2D().copy() + npts)
            ),
        )
        ru = mc.build_polydata(
            np.concatenate((ru.Points.copy(), ud.Points.copy())),
            cells=np.concatenate(
                (ru.GetCells2D().copy(), ud.GetCells2D().copy() + npts)
            ),
        )
        ...
        for space in [modality, "unfold"]:
            for label in labels:
                ...
                if glob.glob(fn1):
                    ...
                    if space != "unfold":
                        s.Points = np.ones(s.Points.shape) * np.nan
                if label == "hipp":
                    nptsHipp = s.n_points
                if space == "unfold":
                    ...
                    if version == "v2" and unfoldAPrescale:
                        raise Warning(
                            "Outputs from hippunfold version 2 does not need rescaling"
                        )
                        unfoldAPrescale = False
                    if unfoldAPrescale:
                ...
                if (
                    label == "dentate"
                ):  # concatenate dentate to hipp (hipp should be immediately prior)
                    s = mc.build_polydata(
                        np.concatenate((oldsurf.Points.copy(), s.Points.copy())),
                        cells=np.concatenate(
                            (
                                oldsurf.GetCells2D().copy(),
                                s.GetCells2D().copy() + nptsHipp,
                            )
                        ),
                    )
                oldsurf = s
...
def area_rescale(vertices, den, label, APaxis=1):
    ...
    surfarea = (
        nib.load(
            f"{resourcesdir}/canonical_surfs/v1/tpl-avg_space-unfold_den-{den}_label-{label}_surfarea.shape.gii"
        )
        .darrays[0]
        .data
    )
    surfarea = hippomaps.utils.density_interp(
        den, "unfoldiso", surfarea.flatten(), label
    )
    ...
    rescalefactor = hippomaps.utils.density_interp(
        "unfoldiso", den, rescalefactor.flatten(), label
    )
