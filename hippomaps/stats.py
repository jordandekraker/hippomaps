...
def spin_test(
    imgfix, imgperm, nperm=1000, metric="pearsonr", label="hipp", version='v1', den="0p5mm"
):
    """
    ...
    version : str, optional
        Hippunfold version parameter for surface plot. Default is 'v1'
    den : str, optional
    ...
    if den != "unfoldiso":
        imgperm = hippomaps.utils.density_interp(
            version, den, "unfoldiso", imgperm, label=label, method="nearest"
        )
        imgfix = hippomaps.utils.density_interp(
            version, den, "unfoldiso", imgfix, label=label, method="nearest"
        )
    if label == "hipp":
    ...
    for ii in range(nperm):
        ...
        permutedimg[:, :, ii] = (
            transrotimg  # this is our permuted image at unfoldiso density
        )

    r_obs = pearsonr(imgfix.flatten(), imgperm.flatten())[0]

    if metric == "pearsonr":
        metricnull = np.corrcoef(
            np.concatenate(
                (imgfix.reshape([-1, 1]), permutedimg.reshape([254 * 126, nperm])),
                axis=1,
            )
        )[0, 1:]
    elif metric == "spearmanr":
        metricnull = spearmanr(
            imgfix.reshape([-1, 1]), permutedimg.reshape([254 * 126, nperm])
        )[0][0, 1:]
...
def moran_test(
    ...
    surf = read_surface(
        f"{resourcesdir}/canonical_surfs/{version}/tpl-avg_space-canonical_den-{den}_label-{label}_midthickness.surf.gii"
    )
    ...
    if metric == "pearsonr":
        metricnull = np.corrcoef(
            np.concatenate((imgfix.reshape([-1, 1]), imgperm_rand), axis=1)
        )[0, 1:]
    elif metric == "spearmanr":
...
def eigenstrapping(
    ...
    **qwargs : dict, optional
    ...
    eigen = SurfaceEigenstrapping(
        surface=f"{resourcesdir}/canonical_surfs/{version}/tpl-avg_space-canonical_den-{den}_label-{label}_midthickness.surf.gii",
        data=imgperm,
        num_modes=num_modes,
        permute=False,
        resample=False,
...
def contextualize2D(
    ...
    if taskMaps.shape[0] != nV:
        taskMapsresamp = hippomaps.utils.density_interp(
            hippomaps.config.get_label_from_nV(taskMaps.shape[0])[1],
            "0p5mm",
            taskMaps,
            label="hipp",
        )
    ...
    def generate_permutations(t):
        hippomaps.utils.blockPrint()
        eigen = SurfaceEigenstrapping(
            surface=f"{resourcesdir}/canonical_surfs/v1/tpl-avg_space-canonical_den-0p5mm_label-hipp_midthickness.surf.gii",
    ...
    APWithTask = np.concatenate(
        (taskMapsresamp, contextHM["AP"].reshape([nV, 1])), axis=1
    )
    AP_R = np.corrcoef(APWithTask.T)[:nT, nT:].flatten()
    AP_p = np.empty((nT))
    for t in range(nT):
        APWithPerm = np.concatenate(
            (contextHM["AP"].reshape([nV, 1]), permutedTasks[:, t, :]), axis=1
        )
    ...
    for t in range(nT):
        metricnull = spearmanr(
            contextHM["subfields_permuted"][:, Subfscorr_best[t]],
            permutedTasks[:, t, :],
        )[0][0, 1:]
    ...
    if n_topComparison > 0:
        for t in range(nT):
            FeatWithTask = np.concatenate(
                (taskMapsresamp[:, t].reshape([nV, 1]), contextHM["featureData"]),
                axis=1,
            )
            ...
            for c in range(n_topComparison):
                ...
                topR[t, c] = R[order[c]]
                FeatWithPerm = np.concatenate(
                    (
                        contextHM["featureData"][:, order[c]].reshape([nV, 1]),
                        permutedTasks[:, t, :],
                    ),
                    axis=1,
                )
    ...
    if plot2D:
        ...
        ax.scatter(
            contextHM["axiscorrAPPD"][0],
            contextHM["subfieldsmaxcorr"],
            c=contextHM["colors"],
            cmap="Set3",
            s=200,
        )
        ...
        if numerbMaps:
            texts = [
                ax.text(
                    contextHM["axiscorrAPPD"][0, f],
                    contextHM["subfieldsmaxcorr"][f],
                    str(int(feature)),
                    ha="center",
                    va="center",
                )
                for f, feature in enumerate(contextHM["feature_n"])
            ]
            for t in range(nT):
                texts.append(
                    ax.text(
            ...
            adjust_text(
                texts, expand=(1, 2), arrowprops=dict(arrowstyle="->", color="gray")
            )
